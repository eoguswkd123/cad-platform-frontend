# Phase 1.2: Three.js Demo (Teapot) Implementation

> **Version**: 0.0.5
> **Last Updated**: 2025-12-17
> **Status**: ✅ 완료 (100%)

---

## Table of Contents (목차)

- [1. Overview (개요)](#1-overview-개요)
- [2. Architecture (아키텍처)](#2-architecture-아키텍처)
- [3. Implementation Checklist (구현 체크리스트)](#3-implementation-checklist-구현-체크리스트)
- [4. Key Implementation Details (핵심 구현 상세)](#4-key-implementation-details-핵심-구현-상세)
- [5. Testing Strategy (테스트 전략)](#5-testing-strategy-테스트-전략)
- [6. Routes & Navigation (라우트 및 네비게이션)](#6-routes--navigation-라우트-및-네비게이션)
- [7. Dependencies & References (의존성 및 참조)](#7-dependencies--references-의존성-및-참조)
- [8. Post-Completion Checklist (완료 후 체크리스트)](#8-post-completion-checklist-완료-후-체크리스트)
- [9. Changelog (변경 이력)](#9-changelog-변경-이력)

---

## Phase Progress (진행 상황)

| Sub-Phase | 상태 | 설명                       |
| --------- | ---- | -------------------------- |
| 1.2.1     | ✅   | TeapotGeometry 기본 렌더링 |
| 1.2.2     | ✅   | 6가지 쉐이딩 모드 구현     |
| 1.2.3     | ✅   | Interactive Controls UI    |
| 1.2.4     | ✅   | OrbitControls 카메라 조작  |

---

## 1. Overview (개요)

### 1.1 목표

Three.js 공식 예제인 [webgl_geometry_teapot](https://threejs.org/examples/#webgl_geometry_teapot)을 React Three Fiber 기반으로 구현하여 3D 렌더링 기초를 학습

### 1.2 MVP 범위

| 항목            | MVP 범위                    | 확장 단계         |
| --------------- | --------------------------- | ----------------- |
| **프레임워크**  | React Three Fiber (R3F)     | -                 |
| **GUI 방식**    | HTML Overlay (Tailwind CSS) | lil-gui 통합      |
| **상태 관리**   | useState (로컬 상태)        | Zustand 전역 상태 |
| **쉐이딩 모드** | 6가지 모드                  | 커스텀 쉐이더     |

### 1.3 기술적 제약

- TeapotGeometry는 `three/addons`에서 import 필요
- R3F 환경에서 Three.js 객체를 JSX로 래핑해야 함

---

## 2. Architecture (아키텍처)

### 2.1 디렉토리 구조

```
src/features/TeapotDemo/
├── index.ts                    # Public exports
├── types.ts                    # 타입 정의 (ShadingMode, TeapotConfig)
├── constants.ts                # 상수 정의 (DEFAULT_TEAPOT_CONFIG)
├── components/
│   ├── index.ts
│   ├── TeapotMesh.tsx          # 3D 메시 렌더링
│   ├── TeapotControls.tsx      # HTML 오버레이 컨트롤
│   └── TeapotScene.tsx         # Scene 구성 래퍼
└── hooks/
    ├── index.ts
    └── useTeapotMaterial.ts    # Material 생성 훅

src/pages/TeapotDemo/
└── index.tsx                   # 페이지 컴포넌트
```

### 2.2 컴포넌트 계층

```
TeapotDemoPage                          [custom] 페이지 컴포넌트
└── TeapotScene                         [custom] 씬 컨테이너
    ├── Canvas                          [@react-three/fiber] 3D 캔버스
    │   ├── PerspectiveCamera           [@react-three/drei] 원근 카메라
    │   ├── OrbitControls               [@react-three/drei] 궤도 컨트롤
    │   ├── ambientLight                [R3F built-in] 환경광
    │   ├── directionalLight (x2)       [R3F built-in] 직사광
    │   ├── TeapotMesh                  [custom] 주전자 메시
    │   │   └── TeapotGeometry          [three/addons] 주전자 형상
    │   └── gridHelper                  [R3F built-in] 바닥 그리드
    └── TeapotControls                  [custom / HTML] UI 컨트롤 패널
```

**범례:**

- `[custom]` - 직접 구현한 컴포넌트
- `[@react-three/fiber]` - R3F 코어
- `[@react-three/drei]` - R3F 헬퍼 라이브러리
- `[R3F built-in]` - Three.js 객체 (R3F에서 JSX로 사용)
- `[three/addons]` - Three.js 확장 모듈

### 2.3 데이터 흐름

```
User Input → TeapotControls → useState(config) → TeapotMesh → Three.js Render

상태 변경 예시:
Shading Mode Select → setConfig({ shadingMode }) → useMemo(material) → mesh.material
```

---

## 3. Implementation Checklist (구현 체크리스트)

### 3.1 Phase 1.2.1: 기본 렌더링

- [✅] TeapotGeometry import 설정
- [✅] TeapotMesh 컴포넌트 생성
- [✅] Canvas 및 기본 조명 설정
- [✅] OrbitControls 통합

### 3.2 Phase 1.2.2: 쉐이딩 모드

- [✅] wireframe 모드 (MeshBasicMaterial)
- [✅] flat 모드 (MeshPhongMaterial, flatShading)
- [✅] smooth 모드 (MeshStandardMaterial)
- [✅] glossy 모드 (MeshPhysicalMaterial)
- [✅] textured 모드 (체커보드 텍스처)
- [✅] reflective 모드 (환경 반사)

### 3.3 Phase 1.2.3: Interactive Controls

- [✅] Tessellation 슬라이더 (1-50)
- [✅] Shading Mode 선택 드롭다운
- [✅] Lid/Body/Bottom 가시성 토글
- [✅] Auto Rotate 체크박스

### 3.4 Phase 1.2.4: 최적화

- [✅] React.memo로 리렌더링 방지
- [✅] useMemo로 Material/Geometry 캐싱
- [✅] HTML Overlay로 3D/UI 분리

---

## 4. Key Implementation Details (핵심 구현 상세)

### 4.1 핵심 코드 패턴

#### 타입 정의

```typescript
// types.ts
export type ShadingMode =
    | 'wireframe'
    | 'flat'
    | 'smooth'
    | 'glossy'
    | 'textured'
    | 'reflective';

export interface TeapotConfig {
    tessellation: number;
    shadingMode: ShadingMode;
    showLid: boolean;
    showBody: boolean;
    showBottom: boolean;
    autoRotate: boolean;
}
```

#### 상수 정의

```typescript
// constants.ts
export const DEFAULT_TEAPOT_CONFIG: TeapotConfig = {
    tessellation: 15,
    shadingMode: 'smooth',
    showLid: true,
    showBody: true,
    showBottom: true,
    autoRotate: true,
};
```

#### TeapotGeometry Import

```typescript
import { TeapotGeometry } from 'three/addons/geometries/TeapotGeometry.js';
```

#### Material Hook 패턴

```typescript
// hooks/useTeapotMaterial.ts
export function useTeapotMaterial(shadingMode: ShadingMode) {
    const material = useMemo(() => {
        switch (shadingMode) {
            case 'wireframe':
                return new THREE.MeshBasicMaterial({ wireframe: true });
            case 'smooth':
                return new THREE.MeshLambertMaterial({ color: 0x156289 });
            case 'glossy':
                return new THREE.MeshPhongMaterial({
                    color: 0x156289,
                    shininess: 100,
                    specular: 0x222222,
                });
            // ... 기타 모드
        }
    }, [shadingMode]);

    return material;
}
```

#### 메시 컴포넌트 패턴

```tsx
// components/TeapotMesh.tsx
export const TeapotMesh = memo(function TeapotMesh({
    config,
}: TeapotMeshProps) {
    const meshRef = useRef<THREE.Mesh>(null);
    const material = useTeapotMaterial(config.shadingMode);

    const geometry = useMemo(() => {
        return new TeapotGeometry(
            3, // size
            config.tessellation, // segments
            config.showBottom,
            config.showLid,
            config.showBody,
            true // fitLid
        );
    }, [
        config.tessellation,
        config.showBottom,
        config.showLid,
        config.showBody,
    ]);

    useFrame((_, delta) => {
        if (config.autoRotate && meshRef.current) {
            meshRef.current.rotation.y += delta * 0.5;
        }
    });

    useEffect(() => {
        return () => geometry.dispose();
    }, [geometry]);

    return <mesh ref={meshRef} geometry={geometry} material={material} />;
});
```

### 4.2 설정 및 상수

| 상수명         | 값        | 설명                   |
| -------------- | --------- | ---------------------- |
| `tessellation` | 15 (기본) | 테셀레이션 레벨 (1-50) |
| `autoRotate`   | true      | Y축 자동 회전          |
| `size`         | 3         | TeapotGeometry 크기    |

---

## 5. Testing Strategy (테스트 전략)

### 5.1 Unit Tests

- [✅] useTeapotMaterial 훅 테스트
- [ ] ShadingMode 타입 검증

### 5.2 Integration Tests

- [ ] TeapotScene 렌더링 테스트
- [ ] Controls 상호작용 테스트

### 5.3 검증 체크리스트

| 검증 항목              | 방법                 | 상태 |
| ---------------------- | -------------------- | ---- |
| 6가지 쉐이딩 모드 전환 | 수동 확인            | ✅   |
| Tessellation 슬라이더  | 수동 확인            | ✅   |
| Lid/Body/Bottom 토글   | 수동 확인            | ✅   |
| Auto Rotate 기능       | 수동 확인            | ✅   |
| OrbitControls 조작     | 수동 확인            | ✅   |
| 반응형 레이아웃        | 수동 확인            | ✅   |
| 타입 체크              | `npm run type-check` | ✅   |
| 린트                   | `npm run lint`       | ✅   |

---

## 6. Routes & Navigation (라우트 및 네비게이션)

| Path           | Component      | Description        |
| -------------- | -------------- | ------------------ |
| `/teapot-demo` | TeapotDemoPage | Teapot 데모 페이지 |

---

## 7. Dependencies & References (의존성 및 참조)

### 7.1 필수 패키지

| 패키지               | 버전     | 용도                             |
| -------------------- | -------- | -------------------------------- |
| `three`              | ^0.181.0 | TeapotGeometry 포함              |
| `@react-three/fiber` | ^8.x     | React Three Fiber                |
| `@react-three/drei`  | ^9.x     | OrbitControls, PerspectiveCamera |

### 7.2 참조 문서

| 문서                                                                           | 역할           |
| ------------------------------------------------------------------------------ | -------------- |
| [Three.js Teapot Example](https://threejs.org/examples/#webgl_geometry_teapot) | 원본 예제      |
| [React Three Fiber Docs](https://docs.pmnd.rs/react-three-fiber)               | R3F 공식 문서  |
| [DEV_GUIDE.md](../../DEV_GUIDE.md)                                             | 폴더 명명 규칙 |

### 7.3 관련 Phase

| Phase     | 관계 | 설명                      |
| --------- | ---- | ------------------------- |
| Phase 1   | 의존 | Foundation 완료 후 시작   |
| Phase 2.1 | 연관 | CADViewer에서 패턴 재사용 |

---

## 8. Post-Completion Checklist (완료 후 체크리스트)

개발 완료 시 Claude가 아래 문서들의 업데이트를 자동으로 제안합니다.

### 8.1 연관 문서 업데이트 (Claude 자동 제안)

| 문서                             | 업데이트 내용                                     |
| -------------------------------- | ------------------------------------------------- |
| `docs/phases/PHASE_NAV_GUIDE.md` | Phase 문서 링크, 상태, 진행률 업데이트            |
| `docs/ROADMAP.md`                | Phase 상태 (🔄→✅), 진행률, Milestones, 변경 이력 |
| `docs/GLOSSARY.md`               | 새로운 용어/약어 추가                             |
| `docs/ARCHITECTURE.md`           | 패키지 구조, 레이어별 역할, 관련 문서 링크        |
| `README.md`                      | 현재 상태, 구현 완료 기능, 개발 예정 목록         |

### 8.2 업데이트 프로세스

1. **구현 완료** → Claude가 "연관 문서 업데이트할까요?" 제안
2. **변경 내용 미리보기** → 각 문서별 수정될 내용 표시
3. **승인** → 사용자 확인 후 일괄 업데이트

---

## 9. Changelog (변경 이력)

| 버전  | 날짜       | 변경 내용                                                                                                               |
| ----- | ---------- | ----------------------------------------------------------------------------------------------------------------------- |
| 0.0.5 | 2025-12-17 | 문서 품질 개선: Material 코드 예제 수정 (실제 구현 반영), Phase 내부 참조 번호 수정 (1.5.x→1.2.x), 테스트 상태 업데이트 |
| 0.0.4 | 2025-12-16 | Phase 번호 변경 (1.5→1.2), 파일명 변경 (1.2_THREEJS_DEMO_TEAPOT.md), 제목 변경 (Three.js Demo (Teapot))                 |
| 0.0.3 | 2025-12-04 | 템플릿 v0.0.3 기준 섹션 순서 변경 (Routes↔Dependencies)                                                                 |
| 0.0.2 | 2025-12-03 | 템플릿 기준 메타데이터 간소화 (Created, Dependencies 제거)                                                              |
| 0.0.1 | 2025-12-02 | 템플릿 적용, 버전 가이드라인 적용 (0.0.x), Section 9 추가                                                               |
| 0.0.0 | 2025-12-01 | 초기 구현 완료                                                                                                          |
