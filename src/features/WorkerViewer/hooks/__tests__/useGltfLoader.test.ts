/**
 * useGltfLoader.test.ts
 * WorkerViewer glTF/GLB 모델 로딩 훅 테스트
 *
 * 주요 테스트:
 * - 초기 상태 검증
...
describe('useGltfLoader', () => {
    ...
    describe('초기 상태', () => {
        it('초기 상태는 status=idle, models=[], selectedModel=null, error=null', () => {
            const { result } = renderHook(() => useGltfLoader());
        ...
        it('반환 타입이 UseGltfLoaderReturn 인터페이스를 준수', () => {
            const { result } = renderHook(() => useGltfLoader());
    ...
    describe('loadModelFromUrl', () => {
        it('loadModelFromUrl 성공 시 selectedModel이 설정됨', () => {
            // 허용된 호스트 (raw.githubusercontent.com) 사용
            const testUrl =
                'https://raw.githubusercontent.com/user/repo/model.glb';
            const createdModel = {
                id: 'url-123',
                name: 'model.glb',
                url: testUrl,
            ...
            act(() => {
                result.current.loadModelFromUrl(testUrl);
            });
        ...
        it('허용되지 않은 호스트 URL 로드 시 에러 설정됨', () => {
            const { result } = renderHook(() => useGltfLoader());
